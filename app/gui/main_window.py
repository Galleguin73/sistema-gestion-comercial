...
from PIL import Image, ImageTk # Importamos las librerías de Pillow
...
class MainWindow(ThemedTk):
    def __init__(self):
        ...
        self.style.map("Nav.TButton",
            ...
            foreground=[('active', self.COLOR_BTN_TEXT), ('!active', self.COLOR_BTN_TEXT)]
        )
        ...
        self.style.map('Treeview', 
                       background=[('selected', self.COLOR_BTN_NORMAL)],
                       foreground=[('selected', self.COLOR_BTN_TEXT)])
    ...
    def crear_botones_navegacion(self):
        ...
        for texto, comando in botones_ordenados:
            btn = ttk.Button(self.nav_frame, text=texto, command=comando, style="Nav.TButton")
        ...
        if config and config.get("logo_path"):
            try:
                ...
                img.thumbnail((180, 100), Image.Resampling.LANCZOS)
                ...
                print(f"Error: No se encontró el archivo de logo en {config['logo_path']}")
    ...
    def actualizar_estado_caja(self, caja_id):
        """Función callback que actualiza el estado de la caja en la ventana principal."""
